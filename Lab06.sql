...
--City that makes the most products: Dallas and Newark
--name and city of cutsomers that live in Dallas and Newark: Basics, Allied. Nobody lives in newarkd
select name 
from customers
where customers.city in(
	select city
	from(
		select products.city, count(city) as sum
		from products
		group by city
		order by sum desc
		limit 2)
...
--Question 3: Display the customers name, pid ordered, and the dollars for all orders, sorted by dollars from high to low
...
--Question 4: Display all customer names in reverse alphabetical order and their total ordered and nothing more. Use coalesce to avoid nulls--


select name, sum(orders.qty) as sum
...
--Customers that bought products from agents based in Tokyo:
...
select orders.cid, agents.aid, agents.name as agentName, customers.name as customer, products.name as productname
...
--Question 6: Write a query to check the accuracy of the dollars column in the Orders table. This means comparing Orders.dollars from data in other tables
--and comparing those values to the values in Orders.dollars. Display all rows in Orders where Orders.dollars is incorrect, if any

select (orders.qty * products.priceUSD) - ((orders.qty * products.priceUSD) * (customers.discount * .01)) as totalCustCost
...
--Whats the difference between a LEFT OUTER JOIN and a RIGHT OUTER JOIN? Give example queries in SQL to demonstrate.
--A left outer join retains all the rows of the left table wheter ther eis a match on the right table. While a right outer join
--retains all rows on the right table whether or not there is a match on the left table. 
